...
public class RedAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Drivetrain drivetrain = new Drivetrain(
                hardwareMap,
                new Pose2D(DistanceUnit.INCH,12, -61, AngleUnit.DEGREES, 180),
                270, true, false
        );
        ...
        actionSet.add( () -> drivetrain.driveToPose(new Pose2D(DistanceUnit.INCH, -5.5, -35, AngleUnit.DEGREES, 180), true) );
        ...
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, -5.5, -40, AngleUnit.DEGREES, 180 ), false ) );
        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 2 );
        drivetrain.stop();

        //Move from chamber to sweep location
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 35, -42, AngleUnit.DEGREES, 180 ), true ) );
        ...
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 34.5, -10, AngleUnit.DEGREES, 90 ), false ) );
        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 1 );
        drivetrain.stop();

        //Strafe to behind block 1
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 46, -10, AngleUnit.DEGREES, 90), true ) );
        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 1 );
        drivetrain.stop();

        //Push block 1 to observation
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 46, -48, AngleUnit.DEGREES, 90), false ) );
...
//        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 3 );
        ...
        //Wait for HP
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 42, -48, AngleUnit.DEGREES, 180), false ) );
        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 3 );
        drivetrain.stop();

        //Get ready for wall pick 1
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 42, -52, AngleUnit.DEGREES, 180), true ) );
        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 3 );
...
for (int i = 0; i < 2; i++) {
        ...
        actionSet.clear();
        int finalI = i;
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, (-3.5 + (finalI * 2)), -35, AngleUnit.DEGREES, 0), false ) );
        actionSet.add( () -> elevator.setElevatorSetpoint(ElevatorConstants.SPECIMEN_READY));
        ...
        actionSet.clear();
        actionSet.add( () -> drivetrain.driveToPose( new Pose2D( DistanceUnit.INCH, 42, -52, AngleUnit.DEGREES, 180), false ) );
        AutoUtil.runActionsConcurrent( actionSet, methodSet1, TimeUnit.SECONDS, 1 );
        drivetrain.stop();
        }
